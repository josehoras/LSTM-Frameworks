...
if load_model:
    ...
    it, loss, smooth_loss, elapsed_time = hist_vars
    data, _, _, _ = aux.load(input_file)
    print(input_dim)
    print(seq_length)
else:
    # load data
...
while True:
    ...
    if it % 100 == 0:
        ...
        print("---")
        data_vars = [input_file, char_to_idx, idx_to_char]
        hyperparams = [seq_length, input_dim, hidden_dim, learning_rate]
        model_vars = [Wx, Wh, Why, b, by]
        hist_vars = [it, loss, smooth_loss, elapsed_time]
        aux.save_model(data_vars, hyperparams, model_vars, hist_vars, t0, data_name)
